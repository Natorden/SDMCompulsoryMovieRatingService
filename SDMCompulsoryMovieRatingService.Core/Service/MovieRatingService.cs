...
namespace SDMCompulsoryMovieRatingService.Core.Service
{
    public class MovieRatingService : IMovieRatingService
    {
        ...
        {
            return _movieRatingRepo.GetAll().Count(r => r.Reviewer == reviewer);
        }
        ...
        public double GetAverageRateFromReviewer(int reviewer)
        {
            var numOfRew = 0;
            var rewSum = 0;
            foreach (var r in _movieRatingRepo.GetAll().Where(r => r.Reviewer == reviewer))
            {
                rewSum += r.Grade;
                numOfRew++;
            }
        ...
        {
            return _movieRatingRepo.GetAll().Count(r => r.Reviewer == reviewer && r.Grade == rate);
        }

        //todo method 4
        public int GetNumberOfReviews(int movie)
        {
            return _movieRatingRepo.GetAll().Count(r => r.Movie == movie);
        }
        ...
        {
            return _movieRatingRepo.GetAll().Count(r => r.Movie == movie && r.Grade == rate);
        }
        ...
        {
            var topRates =
                (_movieRatingRepo.GetAll().GroupBy(movie => movie.Grade).OrderByDescending(x => x.Key)
        ...
        public List<int> GetMostProductiveReviewers()
        {
            var reviewerReviews =
                ...
                group movieRating.Reviewer by movieRating.Reviewer
                into g
            ...
            return (from reviewerCount in reviewerReviews
                where reviewerCount.Count == maxReviews
                select reviewerCount.Reviewer).ToList();
        }
        ...
        public List<int> GetTopRatedMovies(int amount = 1)
        {
            ...
            foreach (var movie in rightAmountMovies) topRatedMovies.Add(movie.Key);
